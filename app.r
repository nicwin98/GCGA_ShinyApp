...
ui <- fluidPage(theme = shinytheme("paper"),
            navbarPage(title = "Differential Expression Analysis - GCGA (NNC9204-0043)",
                  tabPanel(title = "Preface",
                        fluidRow(column(12, wellPanel(tags$h4("Transcriptomic analysis of mice livers chronically activated by a glucagon analogue (GCGA)"),
                                                      tags$br(),
                                                      tags$h6("This app contains information on RNA sequencing data from livers of female mice treated for eight weeks with the glucagon analogue, 
                                                              NNC9204-0043, compared to female mice treated with PBS (control group). For detailed information on the bioinformatic analysis used to generate this data please refer to the abovementioned publication and the R codes available at", tags$a("https://github.com/nicwin98/GCGA_GCGR-Ab_GcgrKO"), ".
                  ...
                  tabPanel(title = "Single Gene Expression - BoxPlot",
                           ...
                           fluidRow(column(12, wellPanel(
                                             textInput(
                                               inputId = "num5",
                                               label = "Find the ENSEMBL ID of your gene (see the table under GCGA vs PBS), enter it below, and click the update button. 
...
server <- function(input, output, server) {
  ...
  output$printboxplot <- downloadHandler(
    filename = function() {
      paste('ExpressionBoxPlot_GCGA', Sys.Date(), '.pdf', sep='')
    },
  ...
  output$printdatatable <- downloadHandler(
    filename = function() {paste('NormCounts_GCGA_', Sys.Date(), '.xlsx', sep='')},
    content = function(file) 
