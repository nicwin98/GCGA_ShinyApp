...
GCGA_PBS_shiny <- read.csv("GCGA_PBS_shiny.csv")
t_box_shiny <- read.csv("t_box_shiny.csv") 
GOBP_GCGA <- read.csv("GOBP_Genes_GCGA_PBS.csv")
GOBP_GCGA$Treatment <- rep("GCGA", length(GOBP_GCGA$Description))
GOBP_GCGA$GeneFraction <- sapply(GOBP_GCGA$GeneRatio, function(x) eval(parse(text=x)))
t_box_shiny$Treatment <- factor(t_box_shiny$Treatment, levels = c("GCGA","PBS"))

ui <- fluidPage(theme = shinytheme("paper"),
            navbarPage(title = "Differential Expression Analysis - GCGA (NNC9204-0043)",
                  tabPanel(title = "Preface",
                        fluidRow(column(12, wellPanel(tags$h4("Transcriptomic analysis of mice livers chronically activated by a glucagon analogue (GCGA) as presented in:"),
                                                      tags$br(),
                                                      ...
                                                      tags$br(),
                                                      tags$h6("This app contains information on RNA sequencing data from livers of female mice treated for eight weeks with the glucagon analogue, 
                                                              NNC9204-0043, compared to female mice treated with PBS (control group). For detailed information on study 
                                                              design and methods please refer to the abovementioned publication and the R codes available at", tags$a("https://github.com/nicwin98/GCGA_GCGR-Ab_GcgrKO"), ".
                                                              Raw data are available through ArrayExpress via the accession code", tags$b("E-MTAB-12040"), ".")))),
                        fluidRow(column(12, wellPanel(
                                                      tags$h5("The app contains three tabs where you can find the following information:"),
                                                      tags$ul(
                                                        tags$li(tags$strong("GCGA vs PBS:")),
                                                        tags$ul(
                                                      ...
                                                      tags$h6("Two other apps have been created in relation to the abovementioned publication:", tags$a("https://weweralbrechtsenlab.shinyapps.io/GCGR_Ab"), "&", 
                                                              tags$a("https://weweralbrechtsenlab.shinyapps.io/GcgrKO"), "."))))),
                  tabPanel(title = "GCGA vs PBS",
                           fluidRow(column(4,
                                wellPanel(
                                sliderInput(
                                 inputId = "num1",
                           ...
                           fluidRow(column(12, wellPanel(tags$h4("Use the table below to search for genes of interest"),
                                                         ...
                                                         tags$h6("Green log 2 fold changes / fold changes means the gene is up-regulated in the GCGA treated mice compared
...
server <- function(input, output, session) {
  ...
  updateSelectizeInput(session, 'gene', choices = as.vector(unique(t_box_shiny$symbol)), selected = character(0), server = TRUE)
  data_box <- reactive({filter(t_box_shiny, symbol == input$gene, .preserve = TRUE)})
  
  reactive_AnalogInhibitor <- reactive({GCGA_PBS_shiny})
  
  output$volcano <- renderPlot({
    EnhancedVolcano(reactive_AnalogInhibitor(),
                    lab = GCGA_PBS_shiny$gene_symbol,
                    x = 'log2FoldChange',
                    y = 'padj',
                    title = 'GCGA vs PBS',
  ...
  output$listDE <- renderDT({datatable(GCGA_PBS_shiny, 
                                     options = list(
                                       scrollX = TRUE,
                                       pageLength = 10,
  ...
  output$printDEselect = downloadHandler('GCGA_Genes_Selected.xlsx', content = function(file) 
  {srows_data <- input$listDE_rows_selected
  writexl::write_xlsx(GCGA_PBS_shiny[srows_data, , drop = FALSE], path = file)
  })
  
  output$expressionboxplot <- renderPlotly({
    validate(
      need(input$gene, "Please select a gene above to generate the boxplot.")
    )
  ...
  output$printboxplot <- downloadHandler(
    filename = function() {
      paste('ExpressionBoxPlot_GCGA_', input$gene, "_", Sys.Date(), '.pdf', sep='')
    },
  ...
  output$printdatatable <- downloadHandler(
    filename = function() {paste0('NormCounts_GCGA_', input$gene, "_",Sys.Date(), '.xlsx', sep='')},
    content = function(file) 
    {writexl::write_xlsx(dplyr::filter(data_box()), path = file)
    })
  
  output$GOBPtable <- renderDT(GOBP_GCGA,
                               filter = "bottom",
                               options = list(
                                 autoWidth = TRUE,
                                 scrollX = TRUE,
  ...
  output$GOBPlot <- renderPlot({
    ...
    GOBP_GCGA <- GOBP_GCGA[srows_data, , drop = FALSE]
    ggplot(GOBP_GCGA, aes(x = Genes, y = Description)) + 
  ...
  output$printGOBPselect = downloadHandler('GCGA_GOBP_Selected.xlsx', content = function(file) 
    {srows_data <- input$GOBPtable_rows_selected
    writexl::write_xlsx(GOBP_GCGA[srows_data, , drop = FALSE], path = file)
    })
  
  output$printGOBPdotplot <- downloadHandler(
    filename = function() {
      paste('DotPlot_GCGA_GOBP', Sys.Date(), '.pdf', sep='')
    },
    content = function(file) {
      srows_data <- input$GOBPtable_rows_selected
      GOBP_GCGA <- GOBP_GCGA[srows_data, , drop = FALSE]
